...
class TextDataSet:
  ...
  def __init__(self, common_params):
    ...
    self.t_record_producer = multiprocessing.Process(target=self._record_producer)
    self.t_record_producer.daemon = True
    self.t_record_producer.start()

    self.t_record_customer=[]
    for i in range(self.thread_num):
      self.t_record_customer.append(multiprocessing.Process(target=self._record_customer))
      self.t_record_customer[-1].daemon = True
