...
float yoloDetector::iou(det_class &det1, det_class &det2)
{
    ...
    float over_w = std::max(0.f, (det1.w+det2.w) - (std::max(det1.x + det1.w, det2.x + det2.w) - std::min(det1.x, det2.x)));
    float over_h = std::max(0.f, (det1.h+det2.h) - (std::max(det1.y + det1.h, det2.y + det2.h) - std::min(det1.y, det2.y)));
...
yoloDetector::yoloDetector(char* cfgfile,char* weightfile,float nms,float thresh,float hier_thresh)
{
    _nms=nms;
    _thresh=thresh;
        ...
        for(int j=0;j<_l.w;j++){
            for(int k=0;k<_l.n;k++){
                ...
                for(int id=0;id<num_class;id++){
                    fclass.push_back(predictions[map_h*map_w * feature_len * k+map_h*map_w * (5+id) +  map_w * i + j]);
                }

                det_class obj;
                obj.x=(j+predictions[map_h*map_w * feature_len * k + map_h*map_w * 0 +  map_w * i + j])/map_w*img.cols;
                obj.y=(i+predictions[map_h*map_w * feature_len * k + map_h*map_w * 1 +  map_w * i + j])/map_h*img.rows;
                obj.w=exp(predictions[map_h*map_w * feature_len * k + map_h*map_w * 2 +  map_w * i + j])*_l.biases[2*k]/map_w*img.cols;
                obj.h=exp(predictions[map_h*map_w * feature_len * k + map_h*map_w * 3 +  map_w * i + j])*_l.biases[2*k+1]/map_h*img.rows;
                obj.id=max_idx(fclass);
                obj.score=fclass[obj.id]*predictions[map_h*map_w * feature_len * k + map_h*map_w * 4 +  map_w * i + j];
...
void yoloDetector::process(cv::Mat &img, std::vector<std::pair<cv::Rect, float> > &out)
{
    out.clear();
