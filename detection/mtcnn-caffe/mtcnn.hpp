...
public:
    ...
    void Detect(cv::Mat &images, std::vector<FaceInf> &face_Inf);
    void Detect(std::vector<cv::Mat> &images, std::vector<std::vector<FaceInf>> &face_Inf);
...
private:
    ...
    void ClassifyFace_1(const std::vector<FaceInfo>& regressed_rects, cv::Mat &sample_single, boost::shared_ptr<caffe::Net<float> >& net, float thresh, std::vector<float> &score);
    void ClassifyFace_2(const std::vector<FaceInfo>& regressed_rects, cv::Mat &sample_single, boost::shared_ptr<caffe::Net<float> >& net, float thresh, std::vector<float> &score, std::vector<bool> &frontalLabel);
