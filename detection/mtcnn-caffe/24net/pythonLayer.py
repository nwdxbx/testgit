...
class Data_Layer_train(caffe.Layer):
    def setup(self, bottom, top):
        self.batch_size = 64
        net_side = 24
        cls_list = '/home/ffh/work/project/mtcnn-caffe/prepare_data/24/cls_24.txt'
        roi_list = '/home/ffh/work/project/mtcnn-caffe/prepare_data/24/roi_24.txt'
        pts_list = '/home/ffh/work/project/mtcnn-caffe/prepare_data/24/pts_24.txt'
        cls_root = '/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/change/'
        roi_root = '/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/change/'
...
class BatchLoader(object):
    def __init__(self,cls_list,roi_list,pts_list,net_side,cls_root,roi_root,pts_root):
        ...
        if imdb_exit:
            #fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/change/24/cls.imdb','r')
            fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/24midimdb_20000/cls.imdb','r')
            self.cls_list = pickle.load(fid)
            fid.close()
        else:
            fid = open('/home/ffh/work/project/mtcnn-caffe/prepare_data/24/cls_24.txt','r')
            lines = fid.readlines()
            fid.close()
            cur_=0
            sum_=len(lines)
        ...
        if imdb_exit:
            #fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/change/24/roi.imdb','r')
            fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/24midimdb_20000/roi.imdb','r')
            self.roi_list = pickle.load(fid)
            fid.close()
        else:
            fid = open('/home/ffh/work/project/mtcnn-caffe/prepare_data/24/roi_24.txt','r')
            lines = fid.readlines()
            fid.close()
            cur_=0
            sum_=len(lines)
        ...
        if imdb_exit:
            #fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/change/24/pts.imdb','r')
            fid = open('/media/ffh/f85082d9-0526-4d9a-89b4-61594dbaa936/mtcnn/data/24midimdb_20000/pts.imdb','r')
            self.pts_list = pickle.load(fid)
            fid.close()
        else:
            fid = open('/home/ffh/work/project/mtcnn-caffe/data/24/pts_24.txt','r')
            lines = fid.readlines()
            fid.close()
            cur_=0
            sum_=len(lines)
...
class regression_Layer(caffe.Layer):
    ...
    def reshape(self,bottom,top):
        ...
        bottom_shape_data = bottom[0].data[self.valid_index]
        self.diff = np.zeros_like(bottom_shape_data, dtype=np.float32)
    ...
    def forward(self,bottom,top):
        ...
        top[0].data[...] = 0
        if self.N != 0:
            self.diff[...] = bottom[0].data[self.valid_index] - np.array(bottom[1].data[self.valid_index]).reshape(bottom[0].data[self.valid_index].shape)
    ...
    def backward(self,top,propagate_down,bottom):
        for i in range(2):
            ...
            if i == 0:
                ...
                sign = -1
            bottom[i].diff[self.valid_index] = sign * self.diff / self.N
...
class cls_Layer(caffe.Layer):
    ...
    def reshape(self,bottom,top):
        ...
        if self.count != 0:
            ...
            top[0].reshape(len(label), 2)
            top[1].reshape(len(label), 1)
    def forward(self,bottom,top):
        top[0].data[...][...]=0
        top[1].data[...][...]=0
        if self.count != 0:
    ...
    def backward(self,top,propagate_down,bottom):
        ...
        if propagate_down[1] and self.count!=0:
            bottom[1].diff[self.valid_index]=top[1].diff[...]
        # print 'bottom[0].data: ', bottom[0].data[self.valid_index]
        # print 'bottom[1].data: ', bottom[1].data[self.valid_index]
        # print 'top[0].data: ',top[0].data
        # print 'top[1].data: ',top[1].data
        # print 'bottom[0].diff: ', bottom[0].diff[self.valid_index]
        # print 'bottom[1].diff: ', bottom[1].diff

class roi_filter_Layer(caffe.Layer):
    def setup(self,bottom,top):
        if len(bottom) != 2:
            raise Exception("Need 2 Inputs")
    def reshape(self,bottom,top):
        ...
        self.N = len(self.valid_index)
        if self.N != 0:
            top[0].reshape(self.N, 4)
            top[1].reshape(self.N, 4)
        else:
            top[0].reshape(len(roi), 4)
            top[1].reshape(len(roi), 4)
    ...
    def forward(self,bottom,top):
        ...
        if self.N != 0:
            ...
            top[1].data[0:self.N] = bottom[1].data[self.valid_index]
    ...
    def backward(self,top,propagate_down,bottom):
        for i in range(2):
            ...
            if not propagate_down[i] or self.N==0:
                continue
            bottom[i].diff[self.valid_index] = top[i].diff[...]
        #print bottom[0].diff

class pts_filter_Layer(caffe.Layer):
    def setup(self,bottom,top):
        if len(bottom) != 2:
            raise Exception("Need 2 Inputs")
    def reshape(self,bottom,top):
        ...
        self.N = len(self.valid_index)
        if self.N != 0:
            top[0].reshape(self.N, 10)
            top[1].reshape(self.N, 10)
        else:
            top[0].reshape(len(pts), 4)
            top[1].reshape(len(pts), 4)
