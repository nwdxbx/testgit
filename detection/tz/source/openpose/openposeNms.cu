...
void nmsGpu(T* targetPtr, int* kernelPtr, const T* const sourcePtr, const T threshold, const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize)
{
    ...
    for (auto n = 0; n < num; n++)
    {
        for (auto c = 0; c < channels; c++)
        {
            ...
            const auto offsetChannel2 = (n * sourceSize[1] + c);
            auto* kernelPtrOffsetted = kernelPtr + offsetChannel2 * imageOffset;
            const auto* const sourcePtrOffsetted = sourcePtr + offsetChannel2 * imageOffset;
            ...
            //     int* kernelPtrOffsetted_local = (int *)targetPtr + n * channels * imageOffset + c * imageOffset;
