...
public:
    ...
    void process(cv::Mat &img,cv::Mat &body, cv::Point offset_pt,std::vector<float> &result);
...
protected:
    ...
    bool detectFace(cv::Mat& img, cv::Mat& body,cv::Point offset_pt, std::vector<cv::Mat>&faces_);
