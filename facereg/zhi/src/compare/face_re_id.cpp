...
{
    static thread_local Face_Re_ID obj("./models/face_reid.prototxt", "./models/face_reid.caffemodel");
    return obj;
}
...
{    
    ...
    resimg=(image-cv::Scalar(127.5, 127.5, 127.5))*0.0078125;
}
...
bool Face_Re_ID::detectFace(cv::Mat& img,cv::Mat& body, cv::Point offset_pt,std::vector<cv::Mat>& faces_)
{
...
//        pt5.y += offset_pt.y;
    ...
    float max_score = .0f;int max_score_index = 0;
    if(rects.size() > 1){
        for(int i=0; i < rects.size();i++)
            if(max_score < faces[i].score){
                max_score_index = i;
...
void Face_Re_ID::process(cv::Mat &img,cv::Mat &body,cv::Point offset_pt,std::vector<float> &result)
{
    std::vector<cv::Mat> faces;
    bool detected = detectFace(img,body,offset_pt,faces);
    //cv::imshow("xxx", faces[0]);
    //cv::waitKey(1000);
    if(detected){
        std::vector< cv::Mat > pkgs;
