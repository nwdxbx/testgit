...
void MTCnn::ClassifyFace_1(const std::vector<MTCnn::FaceInfo>& regressed_rects, cv::Mat &sample_single, std::vector<float> &score)
{
    int numBox = regressed_rects.size();
    Rnet_crop_input_layer = RNet_->input_blobs()[0];

    int input_channel = Rnet_crop_input_layer->channels();
    int input_width   = Rnet_crop_input_layer->width();
    int input_height  = Rnet_crop_input_layer->height();

    Rnet_crop_input_layer->Reshape(numBox, input_channel, input_width, input_height);
    RNet_->Reshape();
    ...
    Rnet_input_data = RNet_->input_blobs()[0]->mutable_cpu_data();
    ...
    for(int k=0;k<numBox;k++)
    {
        ...
        {
            cv::Mat channel(input_width, input_height, CV_32FC1, Rnet_input_data);
            input_channels[k].push_back(channel);
            Rnet_input_data += input_width*input_height;
        }
    ...
    const float *confidence_data = RNet_->output_blobs()[1]->cpu_data();
    const float *reg_data = RNet_->output_blobs()[0]->cpu_data();

    for(int i=0;i<numBox;i++)
    {
       if (confidence_data[2*i+1] > threshold[1])
       {
            FaceRect faceRect;
...
void MTCnn::ClassifyFace_2(const std::vector<MTCnn::FaceInfo>& regressed_rects, cv::Mat &sample_single,
                        std::vector<float> &score , std::vector<bool> &frontalLabel)
{
    int numBox = regressed_rects.size();
    Onet_crop_input_layer = ONet_->input_blobs()[0];

    int input_channel = Onet_crop_input_layer->channels();
    int input_width   = Onet_crop_input_layer->width();
    int input_height  = Onet_crop_input_layer->height();

    Onet_crop_input_layer->Reshape(numBox, input_channel, input_width, input_height);
    ONet_->Reshape();
...
//        cv::waitKey(0);
    ...
    Onet_input_data = ONet_->input_blobs()[0]->mutable_cpu_data();
    std::vector < std::vector<cv::Mat> > input_channels;
    input_channels.resize(numBox);
    for(int k=0;k<numBox;k++)
    {
        for (int i = 0; i < input_channel; ++i)
        {
            cv::Mat channel(input_width, input_height, CV_32FC1, Onet_input_data);
            input_channels[k].push_back(channel);
            Onet_input_data += input_width*input_height;
        }
        cv::split(crop_imgs[k], input_channels[k]);
    }
...
//    const float *reg_box = net->output_blobs()[0]->cpu_data();
    const boost::shared_ptr<caffe::Blob<float> > feature_blob_1 = ONet_->blob_by_name("prob1");
    const boost::shared_ptr<caffe::Blob<float> > feature_blob_2 = ONet_->blob_by_name("conv6-2");
    const boost::shared_ptr<caffe::Blob<float> > feature_blob_3 = ONet_->blob_by_name("conv6-3");
    ...
    const boost::shared_ptr<caffe::Blob<float> > feature_blob_4 = ONet_->blob_by_name("prob10_1");
    const float* data_ptr_direction = NULL;
    data_ptr_direction = (const float *)feature_blob_4->cpu_data();

    const boost::shared_ptr<caffe::Blob<float> > feature_blob_5 = ONet_->blob_by_name("conv7-4");
    const float* data_ptr_toward = NULL;
    data_ptr_toward = (const float *)feature_blob_5->cpu_data();

    for(int i=0;i<numBox;i++)
    {
       if (confidence_data[2*i+1] > threshold[2])
       {
            FaceRect faceRect;
            faceRect.x1 = regressed_rects[i].bbox.x1;
            faceRect.y1 = regressed_rects[i].bbox.y1;
...
void MTCnn::Detect(std::vector<cv::Mat> & images, std::vector<std::vector<FaceInf>> &result_Inf)
{

    std::vector<std::vector<FaceInfo>> total_boxes_item;
    std::vector<std::vector<FaceInfo> > regressed_rects_item;
    ...
    //std::cout<<"ClassifyFace_0"<<std::endl;
    ...
    for(int id_img=0;id_img<img_num;id_img++)
    {
        ...
        if (numBox_item[id_img] != 0)
        {
            ...
            /// Second stage
            ClassifyFace_1(regressed_rects_item[id_img], sample_singles[id_img], score1);
            //std::cout<<"ClassifyFace_1"<<std::endl;
            ...
            {
                ClassifyFace_2(regressed_rects_item[id_img], sample_singles[id_img], score2, isFrontal);
                regressed_rects_item[id_img] = BoxRegress(condidate_rects_, 3);

                //std::cout<<"ClassifyFace_2"<<std::endl;
    ...
    for(int i=0;i<(int)face_Info.size();i++)
    {
        for(int j=0;j<(int)face_Info[i].size();j++)
        {
            ...
            result_Inf[i].push_back(tmp);
            //cv::rectangle(images[i], result_Inf[i][j].faceRect, cv::Scalar(255, 0, 0), 2);
        }
...
void MTCnn::Detect(cv::Mat &images, std::vector<FaceInf> &result_Inf)
{
